...
```bash
# Главная (выбор интерфейса)
open http://localhost:5173

# Касса (для кассира на планшете)
open http://localhost:5173/pos

# Админка (для управляющего с компьютера)
open http://localhost:5173/admin

# Дашборд (для управляющего со смартфона)
...
## Интерфейсы в продакшене

В продакшене будут отдельные домены:
- `pos.mypos.kz` - Касса (только для кассира)
- `admin.mypos.kz` - Админка (только для управляющего)
- `app.mypos.kz` - Дашборд (мобильное приложение)
